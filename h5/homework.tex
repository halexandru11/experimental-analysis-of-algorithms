...
\end{itemize}
Total actual cost for $n$ operations: $\le n + 2n = 3n= O(n)$. \\
...
For a DELETE that triggers contraction:

\[
m = \frac{T}{4} + 1
\]

Before deleting, we are in the second branch ($m < T/2$), so:

\[
\Phi_{i-1} = \frac{T}{2} - m
=
\frac{T}{2} - \left(\frac{T}{4}+1\right)
=
\frac{T}{4}-1
\]

After deleting one element, the number of elements becomes:

\[
m' = \frac{T}{4}
\]

Now the table contracts to:

\[
T' = \frac{T}{2}
\]

and copying cost is $m' = T/4$. Hence the actual cost is:

\[
c_i = 1 + \frac{T}{4}
\]

After contraction, $m' = T'/2$, so:

\[
\Phi_i = 2m' - T' = 2\cdot \frac{T}{4} - \frac{T}{2} = 0
\]
...
\[
\hat{c}_i = \left(1+\frac{T}{4}\right) + 0 - \left(\frac{T}{4}-1\right)
\]
...
For a normal DELETE without resizing, $c_i = 1$ and the potential changes by at most a constant, so its amortized cost is also $O(1)$.
