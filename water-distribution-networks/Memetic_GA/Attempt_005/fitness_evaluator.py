...
class FitnessEvaluator:
    ...
    def _simplified_hydraulic_check(self, diameters: List[float]) -> float:
        ...
        if self.num_pipes <= 10:
            target_idx = max(1, int(0.40 * (n_opts - 1)))
        elif self.num_pipes <= 60:
            target_idx = max(1, int(0.30 * (n_opts - 1)))
        else:
            target_idx = max(1, int(0.20 * (n_opts - 1)))
        ...
        for i, d in enumerate(diameters):
            ...
            if d < target_diameter:
                ...
                weighted_deficit += length_weight * deficit
        ...
        return float(self.reference_cost * (3.0 * weighted_deficit + 15.0 * smallest_share))
