...
class ResultsVisualizer:
    ...
    def plot_published_final_comparison(self):
        ...
        for benchmark in benchmarks:
            ...
            meme_universal = benchmark['summary']['memetic_ga']['mean_universal_score']
            std_universal = benchmark['summary']['standard_ga']['mean_universal_score']

            # Prefer strict paper score, but fall back to universal score when
            # strict evaluation is non-finite (e.g., infeasible -> inf).
            if meme_score is None or not np.isfinite(meme_score):
                meme_score = meme_universal
            if std_score is None or not np.isfinite(std_score):
                std_score = std_universal

            meme_scores.append(float(meme_score))
    ...
    def plot_published_gap_histogram(self):
        ...
        for benchmark in benchmarks:
            ...
            if meme is None or not np.isfinite(meme):
                meme = meme_universal
...
def main():
    ...
    results_file = r"c:\experimental-analysis-of-algorithms\water-distribution-networks\Memetic_GA\Attepmt_002\results\benchmark_results.json"
