...
class BenchmarkRunner:
    ...
    def _trim_redundant_capacity(
        ...
        """
        If a solution is already feasible, try reducing redundant diameter choices
        on lower-importance pipes to cut cost without breaking feasibility.
        """
        diameter_options, _ = self._get_benchmark_cost_spec(network_file)
        options = sorted(float(d) for d in diameter_options)
        current = [float(d) for d in diameters]
        ...
        pipe_order = sorted(
            range(len(current)),
            key=lambda i: self.fitness_evaluator.pipe_importance[i] if i < len(self.fitness_evaluator.pipe_importance) else 1.0
        )
        ...
        while improved and passes < 3:
            ...
            passes += 1
            for i in pipe_order:
                idx = to_index(current[i])
                if idx <= 0:
                    continue

                trial = current.copy()
                trial[i] = options[idx - 1]
                trial_eval = self._evaluate_paper_score(network_file, inp_filepath, network, trial)

                if trial_eval.get('paper_feasible', 0.0) > 0.5 and trial_eval.get('paper_cost', float('inf')) <= current_eval.get('paper_cost', float('inf')):
                    current = trial
        ...
        current_eval.update({
            'trimmed': 1.0,
            'trim_passes': float(passes),
            'trim_note': 'feasible_cost_reduction_pass'
        })
        return current_eval
    ...
    def run_memetic_ga(
        ...
        # The proxy fitness is not identical to the strict paper score, so
        # inspect a small elite set and keep the best final benchmark solution.
        filepath = os.path.join(self.data_dir, network_file)
        elite_candidates = sorted(ga.population, key=lambda ind: ind.fitness)[:min(8, len(ga.population))]
        best_strict_individual = best_individual
        best_strict_eval = self.evaluate_solution(network_file, filepath, best_individual, network)

        for candidate in elite_candidates:
            candidate_eval = self.evaluate_solution(network_file, filepath, candidate, network)
            candidate_score = candidate_eval.get('paper_score', float('inf'))
            if not np.isfinite(candidate_score):
                candidate_score = candidate_eval.get('paper_raw_score', float('inf'))
            best_score = best_strict_eval.get('paper_score', float('inf'))
            if not np.isfinite(best_score):
                best_score = best_strict_eval.get('paper_raw_score', float('inf'))
            if candidate_score < best_score:
                best_strict_individual = candidate
                best_strict_eval = candidate_eval

        runtime = time.time() - start_time

        return best_strict_individual, best_hist, avg_hist, benchmark_hist, runtime
    ...
    def evaluate_solution(
        ...
        if raw_paper_eval['paper_feasible'] > 0.5:
            paper_eval = self._trim_redundant_capacity(network_file, inp_filepath, network, diameters)
            repair_meta = {
                'paper_repaired': 0.0,
                'paper_repair_steps': 0.0,
                'paper_repair_note': 'not_needed',
                'paper_trimmed': paper_eval.get('trimmed', 0.0),
                'paper_trim_passes': paper_eval.get('trim_passes', 0.0),
                'paper_trim_note': paper_eval.get('trim_note', '')
            }
        else:
            ...
            repair_meta = {
                ...
                'paper_trimmed': repaired_eval.get('trimmed', 0.0),
                'paper_trim_passes': repaired_eval.get('trim_passes', 0.0),
                'paper_trim_note': repaired_eval.get('trim_note', '')
            }
    ...
    def run_benchmark(
        ...
        # Per-benchmark configuration tuned for a stronger but still affordable search.
        benchmark_config = {
            'TLN.inp': {
                'population_size': 42,
                'max_generations': 60,
                'local_search_intensity': 0.75
            },
            'hanoi.inp': {
                'population_size': 48,
                'max_generations': 70,
...
def main():
    ...
    results_dir = r"c:\experimental-analysis-of-algorithms\water-distribution-networks\Memetic_GA\Attempt_003\results"
