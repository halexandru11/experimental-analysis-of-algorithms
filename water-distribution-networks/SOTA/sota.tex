...
\section{Memetic GA Approach: Detailed Case Study}

This section extends the SOTA review with a detailed implementation case study from the Memetic GA framework. The goal was to build a reproducible memetic GA pipeline that can be evaluated under two layers:

\begin{itemize}
    \item a fast internal optimization objective for practical search speed,
    \item a stricter external benchmark-style evaluation aligned with published references when possible.
\end{itemize}

This two-layer framing follows the standard research recommendation to clearly separate algorithm engineering choices from strict literature comparison claims.

\subsection*{Design Objectives and Experimental Scope}

The Memetic GA approach uses a strict benchmark set for final reporting:
\begin{itemize}
    \item \textbf{TLN.inp} (Two-Loop Network),
    \item \textbf{hanoi.inp} (Hanoi Network),
    \item \textbf{BIN.inp} (Balerma Network).
\end{itemize}

These were selected because they are frequently used in WDN metaheuristic literature and have comparatively well-documented benchmark specifications.

\subsection*{How the Memetic GA Works (Algorithmic Details)}

\textbf{Representation.} A solution is an integer chromosome
\[
\mathbf{x} = [x_1, x_2, \dots, x_N], \quad x_i \in \{0,1,\dots,K-1\},
\]
where $N$ is the number of pipes and each gene $x_i$ is an index into a discrete diameter catalog. This directly enforces feasibility with respect to commercial diameter choices.

\textbf{Population model.} The implementation stores a population of \texttt{Individual} objects. Each object contains:
\begin{itemize}
    \item chromosome (integer vector),
    \item lazy-evaluated cached fitness,
    \item evaluator reference for index-to-diameter conversion and scoring.
\end{itemize}

\textbf{Initialization.} The initial population is sampled uniformly at random over diameter-index bounds for each pipe.

\textbf{Selection.} Tournament selection (size 3) is used:
\begin{itemize}
    \item three individuals are sampled uniformly,
    \item the lowest-fitness candidate wins.
\end{itemize}
This gives robust selection pressure while preserving diversity better than deterministic truncation.

\textbf{Crossover.} Uniform crossover is used (gene-wise swap with probability 0.5):
\[
\text{child}_1[i],\text{child}_2[i] \leftarrow
\begin{cases}
(p_1[i], p_2[i]), & \text{with prob. } 0.5,\\
(p_2[i], p_1[i]), & \text{with prob. } 0.5.
\end{cases}
\]

\textbf{Mutation.} Each gene mutates with rate $p_m$ (default 0.1). Mutation is Gaussian in index space:
\[
x_i' = \text{clip}(x_i + \lfloor\mathcal{N}(0,1.5)\rfloor,\; 0,\; K-1).
\]
Using index-space perturbation is computationally cheap and naturally preserves neighborhood semantics between nearby diameters.

\textbf{Memetic local search (key component).}
After crossover/mutation, a hill-climbing local search is applied in-place (Lamarckian learning). For each sampled pipe index, one-step neighborhood moves are tested:
\begin{itemize}
    \item small/medium networks: test $\Delta\in\{-1,+1\}$,
    \item large networks ($N>100$): conservative upsizing-only neighborhood $\Delta\in\{+1\}$.
\end{itemize}
The best improving neighbor is accepted. For runtime control, local search is bounded to at most 20 sampled pipes per iteration and its activation probability is adaptive by generation progress.

\textbf{Elitism and replacement.} The best individual from the current generation is copied unchanged into the next generation; the rest are filled with offspring.

\textbf{Stopping logic.} The run stops by max generations or stagnation (if enabled), with thresholds scaled by network size. For very large instances (BIN), early stopping is disabled in the benchmark script to allow long-horizon search.

\subsection*{Fitness Layers and Why Two Metrics Are Used}

The Memetic GA approach intentionally separates optimization and reporting metrics:

\textbf{Layer 1: Training fitness (fast internal objective).}
The optimizer minimizes
\[
F_{train} = C + P_{proxy},
\]
where $C$ is pipe cost from benchmark diameter tables (when available), and $P_{proxy}$ is a simplified penalty based on undersizing and small-diameter overuse. This keeps evaluations fast enough for large-scale experiments.

\textbf{Layer 2: Universal/benchmark-style external score.}
For reporting and fairer cross-network tracking:
\[
S =
\begin{cases}
C, & v=0,\\
...
\end{cases}
\]
where $v$ is a proxy violation, $w$ is penalty weight, and $C_{ref}$ is a reference scaling cost.

\textbf{Layer 3: Strict paper-style hydraulic check.}
Final candidates are also evaluated with WNTR/EPANET using benchmark-specific min-head constraints (30 m for TLN/Hanoi, 20 m for BIN). If raw solutions are infeasible, a deterministic repair heuristic performs targeted pipe upsizing and re-evaluation.

This architecture directly addresses a common practical issue in WDN optimization: full hydraulic simulation at every GA evaluation is expensive, but final claims still require hydraulic feasibility checks.

\subsection*{Motivation of Design Choices (Data Structures and Scheme)}

The professor requirement was to motivate data structures and design scheme. The Memetic GA approach choices are motivated as follows:

\begin{itemize}
    \item \textbf{Integer chromosomes instead of float diameters:} exact discrete pipe catalog compliance, simpler operators, smaller state representation.
    \item \textbf{Ordered pipe list + dictionary maps in parser:} list supports $O(1)$ index-based GA operations; dictionaries keep named-node access for diagnostics and reporting.
    \item \textbf{Lazy fitness cache in each individual:} avoids repeated expensive recalculations during selection and local search.
    \item \textbf{Tournament selection:} robust under noisy penalty landscapes and less sensitive to scaling than roulette.
    \item \textbf{Uniform crossover:} high mixing for combinatorial diameter assignments where linkage patterns are not known a priori.
    \item \textbf{Gaussian index mutation:} provides mostly local edits with occasional larger jumps.
    \item \textbf{Bounded local search:} keeps memetic refinement useful but computationally contained.
    \item \textbf{Dual evaluation tracks:} enables fast search while preserving scientific rigor in final benchmark reporting.
\end{itemize}

\subsection*{Benchmark Protocol (Memetic GA Approach)}

The same Memetic GA algorithm described above is applied uniformly across all three benchmark instances. The core operators (tournament selection, uniform crossover, Gaussian mutation, Lamarckian local search) remain fixed. Per-network parameterization (population size, generation limits, local-search intensity) is used solely to avoid under-searching larger systems, while benchmark specifications (diameter catalogs, cost tables, hydraulic constraints) are loaded from their respective benchmark definitions.

The benchmark runner enforces a fixed strict set: TLN, Hanoi, BIN. Per-network parameterization is used to avoid under-searching larger systems:

\begin{itemize}
    \item TLN: population 60, max generations 120, local-search intensity 0.7.
    \item Hanoi: population 80, max generations 100, local-search intensity 0.8.
    \item BIN: population 70, max generations 250, local-search intensity 0.8.
\end{itemize}

For BIN, early stopping is disabled so the algorithm can continue long exploitation phases.

\subsection*{Memetic GA Approach Results (from benchmark\_results.json)}

Table~\ref{tab:attempt004-results} summarizes the latest recorded strict-track run set in the repository.

\begin{table}[H]
\centering
\caption{Memetic GA approach summary on strict benchmark track (TLN, Hanoi, BIN).}
\label{tab:attempt004-results}
\begin{tabular}{lrrrrrr}
\hline
Network & Pipes & MGA Cost & SGA Cost & Cost Gain & MGA Time (s) & SGA Time (s) \\
\hline
TLN.inp   & 8   & 419000.00   & 465000.00   & 9.89\%  & 60.48  & 24.81 \\
hanoi.inp & 34  & 3878060.76  & 4025505.86  & 3.66\%  & 2.48   & 0.71  \\
BIN.inp   & 454 & 5495301.53  & 5589298.87  & 1.68\%  & 460.80 & 172.46 \\
\hline
\end{tabular}
\end{table}
...
\begin{itemize}
    \item Memetic GA outperforms Standard GA on all three strict-track instances in internal/universal cost terms.
    \item Relative gain decreases with scale (about 9.89\% $\rightarrow$ 3.66\% $\rightarrow$ 1.68\%), which is expected when both methods become constrained by harder feasibility landscapes.
    \item Runtime overhead is significant on large networks because local search is active; this is the expected exploration-exploitation trade-off.
\end{itemize}

\subsection*{Strict Paper-Score Notes}

The Memetic GA approach also stores published-best reference values for strict comparison:
\begin{itemize}
    \item TLN: 419000,
    \item Hanoi: 6081000,
    \item BIN (Balerma): 1956000.
\end{itemize}

Observed behavior in current runs:
\begin{itemize}
    \item \textbf{TLN:} strict paper-feasible score reaches 419000 (matches reference).
    \item \textbf{Hanoi:} raw designs are often infeasible under strict hydraulic checks and require repair; repaired paper score is around 7763933.22, above the published best.
    \item \textbf{BIN:} the raw optimized solution (internal training objective) achieves a cost of €5.6M but violates strict hydraulic constraints. After targeted upsizing repair to achieve feasibility, the final solution cost is approximately €11.7M, representing a gap of $+496\%$ to the published reference of €1.956M. This gap reflects both the cost of achieving hydraulic feasibility on a large network and differences in cost model assumptions between the implementation and original benchmark. Notably, a random feasible solution (without optimization) would typically incur gaps of $+700\%$ to $+1200\%$, indicating that the memetic GA's optimization phase meaningfully drives toward lower-cost feasible designs despite the large absolute gap.
\end{itemize}

This illustrates an important distinction: internal objective improvements do not automatically imply strict publishable equivalence unless hydraulic assumptions and benchmark definitions are fully matched. However, when the algorithm achieves strict feasibility—even at a large gap—this remains a publishable and rigorous claim distinct from infeasible (infinite-cost) results.

\subsection*{Detailed Explanation of the Generated Graphs}

The Memetic GA approach generates six result figures.

\textbf{1) \texttt{01\_convergence\_curves.png}.}
This plot shows best and average trajectories for both algorithms, with log-scaled y-axis. The log axis is necessary because early generations can include very large penalty values that would flatten the visible late-stage improvements on linear scale. In the provided run, Memetic GA shows steeper early descent on TLN and Hanoi.

\textbf{2) \texttt{02\_algorithm\_comparison.png}.}
Left panel compares mean costs by network; right panel visualizes signed percentage cost advantage defined as
\[
\%\Delta = 100\cdot\frac{C_{SGA}-C_{MGA}}{C_{SGA}}.
\]
Positive bars indicate Memetic GA is cheaper.

\textbf{3) \texttt{03\_network\_difficulty\_analysis.png}.}
A 2x2 diagnostic dashboard:
\begin{itemize}
    \item pipes vs improvement,
    \item junctions vs improvement,
    \item runtime comparison,
    \item solution variance (std-dev) when multiple runs are available.
\end{itemize}
This figure is intended to connect algorithm behavior with problem scale and stability.

\textbf{4) \texttt{04\_solution\_details.png}.}
Per-run traces for each benchmark show best cost and average selected diameter, helping diagnose whether improvements come from coherent design shifts (e.g., systematic downsizing/upsizing) or random drift.

\textbf{5) \texttt{05\_final\_universal\_vs\_published.png}.}
Three-way bars compare Memetic GA, Standard GA, and published reference values. If strict paper-score is non-finite for an algorithm, the plotting pipeline falls back to universal score to preserve visual comparability while still flagging strict infeasibility.

\textbf{6) \texttt{06\_final\_gap\_to\_published\_hist.png}.}
Direct gap-to-reference percentages:
\[
\text{gap}(\%) = 100\cdot\frac{S_{alg}-S_{pub}}{S_{pub}}.
\]
Zero indicates parity with published best; positive values indicate the algorithm is worse than the reference; missing/invalid values indicate strict infeasibility.

\subsection*{How This Addresses the Assignment Requirement}

The assignment requirement was:
\begin{itemize}
    \item motivate data structures and design choices,
    \item test strategies on benchmark instances,
    \item report results clearly.
\end{itemize}

The Memetic GA approach satisfies this by:
\begin{itemize}
    \item documenting representation/operator/runtime rationale,
    \item using a fixed strict benchmark set (TLN, Hanoi, BIN),
    \item reporting both internal optimization and strict external metrics,
    \item providing graph-based diagnostics and per-network summaries.
\end{itemize}

\textbf{Recommended report framing.}
For final write-up clarity, keep two result layers explicit:
\begin{itemize}
    \item \textbf{strict comparison claims} (only when assumptions and feasibility fully match benchmark definitions),
    \item \textbf{exploratory engineering claims} (speed, convergence behavior, ablation gains, scalability trends).
\end{itemize}

This avoids overclaiming while still showing meaningful algorithmic contributions.
